...
def load_qa_pipeline():
    ...
    print("Loading Question-Answering LLM pipeline...")
    qa_pipeline = pipeline("question-answering", model="HuggingFaceH4/zephyr-7b-beta")
...
def format_dashboard_data_for_llm(assets, student_id):
    """
    Consolidates all the key dashboard information for a student into a single text string
    to be used as context for the LLM.
    """
    print(f"Formatting dashboard data into text context for student {student_id}...")

    # Get all the generated dataframes
    grad_summary_df = assets['grad_summary']
    ml_recs = generate_ml_recommendations(student_id, assets)
    collab_recs = get_collaborative_recommendations(student_id, assets)
    illuminate_data = assets['illuminate'][assets['illuminate']['student_id'] == str(student_id)]

    # Start building the context string
    context = f"Student Academic Report for ID: {student_id}\n\n"

    # Graduation Progress Summary
    context += "--- Graduation Progress ---\n"
    student_needs = grad_summary_df[grad_summary_df['student_id'] == str(student_id)]
    if not student_needs.empty:
        for _, row in student_needs.iterrows():
            if row['AreaCreditStillNeeded'] > 0:
                context += f"For {row['SubjectArea']}, the student still needs {row['AreaCreditStillNeeded']} credits.\n"
            else:
                context += f"The {row['SubjectArea']} requirement is complete.\n"
    else:
        context += "No graduation summary data is available.\n"

    context += "\n"

    # AI-Powered Recommendations
    context += "--- AI Course Recommendations ---\n"
    if not ml_recs.empty:
        for subject_area in ml_recs['SubjectArea'].unique():
            context += f"For {subject_area}:\n"
            subject_df = ml_recs[ml_recs['SubjectArea'] == subject_area].sort_values(by='success_prob',
                                                                                     ascending=False).head(3)
            for _, row in subject_df.iterrows():
                context += f"- Course: {row['coursename']}, Predicted Success: {row['success_prob'] * 100:.0f}%\n"
    else:
        context += "No AI-powered recommendations were generated.\n"

    context += "\n"

    # Peer-Based Recommendations
    context += "--- Courses Popular with Academic Peers ---\n"
    if not collab_recs.empty:
        for _, row in collab_recs.head(5).iterrows():
            context += f"- {row['coursename']} was taken successfully by {row['peer_count']} similar students.\n"
    else:
        context += "No peer-based recommendations were generated.\n"

    context += "\n"

    # Recent Assessment Scores
    context += "--- Recent Assessment Scores ---\n"
    if not illuminate_data.empty:
        latest_assessments = illuminate_data.sort_values('responsedatevalue').groupby('Department').last()
        for dept, row in latest_assessments.iterrows():
            context += f"- In {dept}, the most recent assessment score was {row['Response_percent_correct']:.1f}%.\n"
    else:
        context += "No recent assessment data is available.\n"

    print("Context string created successfully.")
    return context
...
# ---- Load the LLM ----
qa_llm = load_qa_pipeline() if all_assets_loaded else None
...
# --- Sidebar button sets session state ---
if st.sidebar.button("View Performance", key="view_performance_button"):
    if all_assets_loaded and qa_llm:
        if student_id_input:
            st.session_state.show_dashboard = True
            st.session_state.active_student_id = student_id_input
            st.session_state.dashboard_context_string = format_dashboard_data_for_llm(
                all_assets_loaded, student_id_input
            )
        else:
...
if st.session_state.get("show_dashboard", False):
    st.header(f"🚀 Performance Overview for Student ID: {st.session_state.active_student_id}")
...
# --- Chat Assistant (Always Active After Dashboard Shows) ---
if st.session_state.get("show_dashboard", False):
    st.divider()
    st.subheader("💬 Ask About Your Performance Data")
    ...
    user_input = st.chat_input("Ask a question about your academic progress...")
    ...
    if user_input:
        ...
        with st.spinner("Thinking..."):
            response = qa_llm(
                question=user_input,
                context=st.session_state.dashboard_context_string
            )
            answer = response["answer"] if response else "I couldn't generate an answer from the data."
